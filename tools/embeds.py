...
def make_embed(title: str = None, description: str = None, ctx: Context = None, color='dark_theme',
          image_url: str = None, author=True) -> Embed:
    ...
    if isinstance(color, str) and color.lower() in colors:
        embed = Embed(color=colors[color.lower()],
...
async def error_message(description: str, ctx: Context, author:bool=True):
    ...
    Args:
        ...
        author (bool, optional): Whether or not you wish to set the author of embed. Defaults to True.
    """
    await ctx.send(embed=make_embed(title='ERROR', description=f'📢 **{description}**', ctx=ctx, color='dark_red', author=author), delete_after=30)

def error_embed(title: str, description: str, ctx: Context, author:bool=True) -> Embed:
    ...
    Returns:
        Embed: discord embed object.
    """
    return make_embed(title=f'ERROR: {title}', description=f'📢 **{description}**', ctx=ctx, color='dark_red', author=author)
...
def files_and_links_only(ctx: Context) -> Embed:
    ...
    embed = make_embed(description="This channel is for submissions only! All messages that do not contain an image or a link are automatically removed.", ctx=ctx, color="reddit")
