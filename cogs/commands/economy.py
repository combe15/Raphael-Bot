...
class Economy(Cog):
    ...
    async def balance(self, ctx: Context, member: discord.Member = None) -> None:
        ...
        embed = embeds.make_embed(
            ctx=ctx, image_url="https://subby.dev/i/icons/atm.png"
        )
        ...
        if member is ctx.author:
            ...
            embed.description = (
                f"{member.mention} has {subby_api.get_balance(member.id):,.1f} :ramen:"
            )
    ...
    async def payment(self, ctx: Context, member: discord.Member, amount: int) -> None:
        ...
        subby_api.record_ledger(ctx.author.id, member.id, amount, "Member payment")

        embed = embeds.make_embed(
            ctx=ctx,
            title="Payment",
            description=f"{ctx.author.mention} has paid {member.mention}\n*Amount*={amount:,} :ramen:",
        )
    ...
    async def set_balance(
        ...
        embed = embeds.make_embed(
            ...
            description=f"{user.mention}'s balance has been set to {amount:,.1f} :ramen: "
            f"from {subby_api.get_balance(user.id):,.1f} :ramen:",
        )
        embed.set_thumbnail(url="https://subby.dev/i/icons/atm.png")
    ...
    async def give_balance(
        ...
        embed = embeds.make_embed(
            ...
            f"New balance {subby_api.get_balance(user.id):,.1f}:ramen:",
        )
        if reason:
