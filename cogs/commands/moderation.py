...
class ModerationCog(Cog):
    ...
    async def ban_member(self, ctx: Context, member: discord.Member, *, reason: str):
        ...
        with dataset.connect(database.get_db()) as db:
            db["mod_logs"].insert(dict(
                user_id=member.id, mod_id=ctx.author.id, timestamp=ctx.message.created_at, reason=reason, type="ban"
            ))
    ...
    async def unban_member(self, ctx: Context, user: discord.User, *, reason: str):
        ...
        with dataset.connect(database.get_db()) as db:
            db["mod_logs"].insert(dict(
                user_id=user.id, mod_id=ctx.author.id, timestamp=ctx.message.created_at, reason=reason, type="unban"
            ))
    ...
    async def kick_member(self, ctx: Context, member: discord.Member, *, reason: str):
        ...
        with dataset.connect(database.get_db()) as db:
            db["mod_logs"].insert(dict(
                user_id=member.id, mod_id=ctx.author.id, timestamp=ctx.message.created_at, reason=reason, type="kick"
            ))
    ...
    async def mute(self, ctx: Context, member: discord.Member, *, reason: str):
        ...
        with dataset.connect(database.get_db()) as db:
            db["mod_logs"].insert(dict(
                user_id=member.id, mod_id=ctx.author.id, timestamp=ctx.message.created_at, reason=reason, type="mute"
            ))
    ...
    async def unmute(self, ctx: Context, member: discord.Member, *, reason: str):
        ...
        with dataset.connect(database.get_db()) as db:
            db["mod_logs"].insert(dict(
                user_id=member.id, mod_id=ctx.author.id, timestamp=ctx.message.created_at, reason=reason, type="unmute"
            ))
    ...
    async def warn(self, ctx: Context, member: discord.Member, *, reason: str):
        ...
        with dataset.connect(database.get_db()) as db:
            db["mod_logs"].insert(dict(
                user_id=member.id, mod_id=ctx.author.id, timestamp=ctx.message.created_at, reason=reason, type="warn"
            ))
    ...
    async def add_note(self, ctx: Context, member: discord.Member, *, note: str):
        ...
        with dataset.connect(database.get_db()) as db:
            db["mod_notes"].insert(dict(
                user_id=member.id, mod_id=ctx.author.id, timestamp=ctx.message.created_at, note=note
            ))
