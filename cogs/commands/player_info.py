...
class PlayerInfo(commands.Cog):
    ...
    async def info(self, ctx, member: discord.Member = None):
        ...
        embed = embeds.make_embed(ctx=ctx, title=f'{member.display_name}\'s User Info', image_url=member.avatar_url, color='blurple',
    ...
    async def pfp(self, ctx, user: discord.User = None):
        ...
        embed = embeds.make_embed(ctx=ctx, description=f"[{user.name}]({user.avatar_url})")
    ...
    async def check_permissions(self, ctx, member: discord.Member = None):
        ...
        embed = embeds.make_embed(ctx=ctx, title='Permissions for:', description=ctx.guild.name, colour=member.colour)
    ...
    async def lastMessage(self, ctx, *channel_ids: commands.TextChannelConverter):
        async with ctx.channel.typing():
            f = open("names.csv", "w")
            f.write(f"Author_ID,Author_Name,Date,Roles\n")
    ...
    @commands.is_owner()
    @commands.command()
    async def lastContrib(self, ctx, channel: commands.TextChannelConverter, *contributors: discord.Member):
        async with ctx.channel.typing():
            f = open("namesContrib.csv", "w")
            f.write(f"Author_ID,Author_Name,Date\n")
            ...
            for message in fetchMessage:
                if message.author.id not in server_members:
                    continue
                if server_members[message.author.id] == "NULL" or message.created_at > server_members[message.author.id]:
                    server_members.update({message.author.id: message.created_at})
    ...
    async def purge(self, ctx: commands.context, *members: discord.Member):
        for member in members:
            try:
                ...
                await ctx.send(f"Member {member.mention} - {member.id} Failed to kick\n{e}")
                continue
