...
class Shop(Cog):
    ...
    @emote.command(name="buy", aliases=['add'],
    ...
    async def buy(self, ctx: Context, emote_name: str, image_url: str) -> None:
        ...
        if (image_url.endswith(".gif")and emoji_count["animated"] <= ctx.guild.emoji_limit):
            pass  # do nothing
        elif emoji_count["non_animated"] <= ctx.guild.emoji_limit:
            pass  # do nothing
        else:
            await embeds.error_message(ctx=ctx, description="Server is at emoji limit. You must delete an emoji to add more.")
            return

        # Check if user can buy emoji
        if (bal := subby_api.get_balance(ctx.author.id)) < BUY_EMOJI:
            await embeds.warning_message(ctx=ctx, description=f"Insufficient funds, you need {BUY_EMOJI} :ramen:\nYour balance: {bal:,}")
            log.info( f"Emote.buy: Error {ctx.author.name} didn't meet funds of {DELETE_EMOJI}, while {bal=}")
            return

        try:
            response = requests.get(image_url, stream=True)
            emote = await ctx.guild.create_custom_emoji(name=emote_name, image=response.content)
        except discord.Forbidden:
            await embeds.error_message(ctx=ctx, description="Bot must have the manage_emojis permission to do this.")
            log.info(f"Emote.buy: Error {ctx.author.name} bot doesn't have manage_emojis permission.")
            return
        except discord.HTTPException as e:
            await embeds.error_message(ctx=ctx, description=f"An error occurred creating an emoji.\n {e.text}")
            log.info(f"Emote.buy: Error {ctx.author.name} HTTPException, {e.text}.")
            return

        subby_api.subtract_balance(ctx.author.id, BUY_EMOJI, True)
        subby_api.record_ledger(ctx.author.id, ctx.me.id, BUY_EMOJI, f"Created emoji {emote}")
        subby_api.record_emoji(ctx.author.id, emote.name, 'purchase', BUY_EMOJI)
        log.info(f"Emote.buy: {ctx.author.name}'s money was taken out of their account'")
    ...
    async def delete(self, ctx: Context, emote: discord.Emoji) -> None:
        ...
        if emote.guild.id != ctx.guild.id:
            await embeds.error_message(ctx=ctx, description="This emoji is not from this server.")
        ...
        if (bal := subby_api.get_balance(ctx.author.id)) < DELETE_EMOJI:
            await embeds.warning_message(ctx=ctx, description=f"Insufficient funds, you need {DELETE_EMOJI} :ramen:\nYour balance: {bal:,}")
            log.info(f"Emote.delete: Error {ctx.author.name} didn't meet funds of {DELETE_EMOJI}, while {bal=}")
            return
        ...
        try:
            ...
            await embeds.error_message(ctx=ctx, description="Bot must have the manage_emojis permission to do this.")
            log.error(f"Emote.delete: Error {ctx.author.name} bot doesn't have manage_emojis permission.")
            return
        except discord.HTTPException as e:
            await embeds.error_message(ctx=ctx, description=f"An error occurred removing an emoji.\n {e.text}")
        ...
        subby_api.record_ledger(ctx.author.id, ctx.me.id, DELETE_EMOJI, f"Removed emoji {emote}")
        subby_api.record_emoji(ctx.author.id, emote.name, 'removal', DELETE_EMOJI)
        log.info(f"Emote.delete: {ctx.author.name}'s money was taken out of their account'")
    ...
    @commands.cooldown(3, 86400, type=BucketType.member) # 1 day = 86400 seconds
    @emote.command(name="rename", aliases=['name'], 
    ...
    async def rename(self, ctx: Context, emote: discord.Emoji, new_name: str) -> None:
        ...
        if (bal := subby_api.get_balance(ctx.author.id)) < RENAME_EMOJI:
            await embeds.warning_message(ctx=ctx, description=f"Insufficient funds, you need {RENAME_EMOJI} :ramen:\nYour balance: {bal:,}")
            log.info(f"Emote.rename: Error {ctx.author.name} didn't meet funds of {RENAME_EMOJI}, while {bal=}")
            return

        try:
            await emote.edit(name=new_name, reason=f"{ctx.author.name} paid to rename emoji.")

        except discord.Forbidden:
            await embeds.error_message(ctx=ctx, description="Bot must have the manage_emojis permission to do this.")
            log.error(f"Emote.rename: Error {ctx.author.name} bot doesn't have manage_emojis permission.")
            return
        except discord.HTTPException as e:
            await embeds.error_message(ctx=ctx, description=f"An error occurred renameing an emoji.\n {e.text}")
        ...
        subby_api.record_ledger(ctx.author.id, ctx.me.id, RENAME_EMOJI, f"Renamed emoji {emote}")
        log.info(f"Emote.rename: {ctx.author.name}'s money was taken out of their account'")
