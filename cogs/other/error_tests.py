...
class ErrorTests(Cog):
    ...
    async def too_many_arguments(self, ctx: Context):
        """Exception raised when the command was passed too many arguments and
        its Command.ignore_extra attribute was not set to True."""
    ...
    async def bad_argument(self, ctx: Context):
        """Exception raised when a parsing or conversion failure is encountered on an argument
        to pass into a command."""
    ...
    async def invalid_end_of_quoted_string_error(self, ctx: Context):
        """An exception raised when a space is expected after the closing quote in a string but
    ...
    async def command_registration_error(self, ctx: Context):
        """An exception raised when the command can’t be added because the name is already taken by
        a different command."""
...
def setup(bot: Bot) -> None:
    ...
    bot.add_cog(ErrorTests(bot))
    log.info("Cog loaded: ErrorTests")
